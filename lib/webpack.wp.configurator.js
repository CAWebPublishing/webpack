...
const serializeData = ( data ) => {
  Object.entries(data).forEach(([k, v]) => {
      ...
      switch( typeof v ){
        case 'string':
          v = v.match(/(browserslist:)?.*\/node_modules\/(@wordpress\/|@svgr\/)?([\w-]+)\/.*/g) ?
            v.replace(/(browserslist:)?.*\/node_modules\/(@wordpress\/|@svgr\/)?([\w-]+)\/.*/g, '$1$2$3') : v;
          ...
          // all of these values can be wrapped in a require.resolve since they are all packages that need to be resolved
            ...
            if( 'react-refresh/babel' === v ){
              v = `hasReactFastRefresh && require.resolve( '${v}' )`;
...
let constants = Object.entries({
  // 'webpackCommand': "'build' === process.argv[2] ? 'build' : 'serve'",
  'isProduction': "getArgVal('mode', 'development') === 'production'",
  'hasReactFastRefresh': "getArgVal('hot') && ! isProduction",
  'require': 'createRequire(import.meta.url)',
  'externals': "getArgVal('externals', 'true') === 'true'",
...
let formattedCode = await prettier.format(
  ...
  ${imports}\n
  ${constants}\n
  export default {${output.join(',').replace(/([{[(]),/g, '$1').replace(/(],)/g, '].filter(Boolean),')}}`,
  {
