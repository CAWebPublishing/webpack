...
(() => {
  ...
  const plugin = (...pluginArgs) => {
    ...
    const listAccordions = listAccordionArgs => {
      ...
      const node =
        listAccordionArgs.length <= 0 ? document.documentElement : listAccordionArgs[0];
  ...
  window.addEventListener('load', () => {
    ...
    //Globally set var
    const reMainNav =
      typeof defaultActiveLink !== 'undefined' ? new RegExp(`^${defaultActiveLink}$`, 'i') : null; // Regex for finding the index of the default main list item
